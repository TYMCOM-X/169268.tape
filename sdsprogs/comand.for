...
C
      INTEGER LISPNT(51),ENTPNT(301),MINTYP(300),ALLCOM(450)
      INTEGER IPAR(50),INLINE(80)
      COMMON /COMPKG/ IP,LISMAX,IPAR,INLINE,LISPNT,ENTPNT,MINTYP
      COMMON /COMPKG/ ALLCOM
C
      IF(INITIA.EQ.1) GO TO 1
      INITIA=1
      IPAR(2)=LOCF(INLINE)
      CALL INIT(IPAR)
C
...
C
      DO 40 J=1,IPAR(3)
      IJ=J
      IF(ICHAR(INLINE,IJ).EQ.IPAR(24)) GO TO 50
40    CONTINUE
C
      GO TO 60
50    IPAR(3)=J-1
...
3     FORMAT('+',A5,$)
C
5     READ(IPAR(1),10,END=100) (INLINE(I),I=1,80)
10    FORMAT(80A5)
C
C    SET LINE SCANNING POINTER TO 1
      IP=1
...
18    K=K*5
      DO 20 J=K,1,-1
      IJ=J
      IF(ICHAR(INLINE,IJ).NE.SPACE) GO TO 30
20    CONTINUE
30    LOL=J
...
C    READ A NEW LINE OF DATA INTO ARRAY JNLINE
      READ(IPAR(1),10,END=100)(JNLINE(I),I=1,80)
...
33    K=K*5
      DO 34 K1=K,1,-1
      IJ=K1
      IF(ICHAR(JNLINE,IJ).NE.SPACE) GO TO 35
34    CONTINUE
...
C
      RETURN
C
C                       IF AN EOF WAS ENCOUNTERED, COME HERE, AND
C                       EXECUTE A GOTO.
C
100     GOTO EOF
C
      END
      SUBROUTINE FILOLD(FILNAM,USNAME,PROJ,PROG,RETURN)
C
...
C    CHECK FOR SPECIFICATION ERRORS
      ...
      MX=LISPNT(LIST+1)-LISPNT(LIST)
      IF(ENTRY.LE.0 .OR. ENTRY.GT.MX) GO TO 100
C
C    COMPUTE LENGTH OF COMMAND ENTRY
      LENG=ENTPNT(LISPNT(LIST)+ENTRY)-ENTPNT(LISPNT(LIST)+ENTRY
     1-1)
C
...
10    IP=IP+1
      ...
      NEXTCH=ICHAR(INLINE,IP)
      IF(ISPACE.NE.0) RETURN
      IF(NEXTCH.EQ.SPACE) GO TO 10
      RETURN
C
20    NEXTCH=-1
      RETURN
      END
      SUBROUTINE LINSAV(IARRAY)
C
...
C
C
      SUBROUTINE LNRSTR(IARRAY)
C
C    THIS ROUTINE IS THE COMPLEMENT OF ROUTINE LINSAV, IN THAT
...
C    COMMAND.
C
      CALL CMND(LIST,IW,ISTAT)
      KOM=ISTAT
      RETURN
      END
      SUBROUTINE CMND(LISTNO,IW,ISTAT)
C
...
C    COMPUTE NUMBER OF ENTRIES IN LIST NUMBER 'LIST'
      NUMENT=LISPNT(LIST+1)-LISPNT(LIST)
C
...
C    COMPUTE MX, THE MAXIMUM NUMBER OF CHARACTERS IN ENTRY 'I'
      MX=ENTPNT(LISPNT(LIST)+I)-ENTPNT(LISPNT(LIST)+I-1)
...
C    NO, BUT IT MUST HAVE A MINIMUM LENGTH
      IF((IPREL-1).GE.MINTYP(LISPNT(LIST)+I-1)) GO TO 90
C
...
C    FIRST COMPUTE THE ENTRY POINTER, J
   10 J=ENTPNT(LISPNT(LIST)+ENTRY-1)
C
C    ADD CHARACTER POSITION AND GET CHARACTER
      LCH=ICHAR(ALLCOM,J+POSN-1)
C
      RETURN
      END
...
C    BE PICKED UP, IN CHARACTERS.  IF MAXLEN IS POSITIVE, INITIA
...
C    ARROW WILL BE SENT TO THE TRMNL.  IF 'LOC' IS POSITIVE, NO
...
40    CALL CPRINT(ARROW)
      ...
      RETURN
      END
      SUBROUTINE WRINLN(UNIT,BEGIN,END)
C
...
100   DO 110 J=1,5
      IJ=J
      IF(IC.EQ.ICHAR(IPAR(27),IJ)) GO TO 130
110   CONTINUE
C
120   DELIM=.FALSE.
      RETURN
...
130   DELIM=.TRUE.
      RETURN
      END
      SUBROUTINE PUTCHL(LIST,ENTRY,POSN,NEWCHR)
C
      INTEGER LISPNT(51),ENTPNT(301),MINTYP(300),ALLCOM(450)
      INTEGER IPAR(50),INLINE(80)
      COMMON /COMPKG/ IP,LISMAX,IPAR,INLINE,LISPNT,ENTPNT,MINTYP
...
10    FORMAT(' BAD "LIST" ARGUMENT IN SUBROUTINE PUTCHL:',I3)
      RETURN
C
20    MAXENT=LISPNT(LIST+1)-LISPNT(LIST)
      JENT=LISPNT(LIST)+ENTRY-1
      IF(ENTRY.GT.0 .AND. ENTRY.LE.MAXENT) GO TO 40
      TYPE 30,ENTRY
30    FORMAT(' BAD "ENTRY" ARGUMENT IN SUBROUTINE PUTCHL:',I3)
      RETURN
C
40    MAXPOS=ENTPNT(JENT+1)-ENTPNT(JENT)
      IF(POSN.GT.0 .AND. POSN.LE.MAXPOS) GO TO 60
      TYPE 50,POSN
50    FORMAT(' BAD "POSN" ARGUMENT IN SUBROUTINE PUTCHL:',I3)
      RETURN
C
60    IC=ENTPNT(JENT)+POSN-1
